...
import {
  editBuyerSchema,
  EditBuyerFormValues,
} from "@/components/forms/buyers/edit-modal/edit.validation";
...
import {
  useUpdateBuyer,
  useUploadDocument,
} from "@/services/mutation/buyer.mutation";
...
export default function EditBuyerModal({
  ...
  const handleRemoveSocial = (socialToRemove: {
    name: string;
    url: string;
  }) => {
    const updatedSocials = socials.filter(
      (s) => !(s.name === socialToRemove.name && s.url === socialToRemove.url)
    );
    setValue("socials", updatedSocials);
  };
  ...
  const handleFileRemoved = (fileToRemove: DocumentUploadResponse) => {
    setValue(
      "files",
      files.filter((file) => file.document_id !== fileToRemove.document_id)
    );
  };
  ...
  const onSubmit = async (values: EditBuyerFormValues) => {
    ...
    console.log("Update Payload:", updateBuyerPayload);
    console.log("Buyer ID:", buyerData.buyer_id);
                        ...
                        >
                          ...
                          <SelectContent>
                            {SOCIAL_OPTIONS.filter(
                              (option) =>
                                !socials.some(
                                  (social) => social.name === option.value
                                )
                            ).map((option) => (
                              <SelectItem
                                key={option.value}
                        ...
                        <div className="flex flex-wrap gap-2">
                          {socials.map((social, index) => {
                            const socialOption = SOCIAL_OPTIONS.find(
                              (s) => s.value === social.name
                            );
                            const IconComponent =
                              socialOption?.icon || SOCIAL_OPTIONS[0].icon;
                            return (
                              <div
                              ...
                              >
                                <div
                                  className={`w-6 h-6 rounded-full ${socialOption?.color} flex items-center justify-center`}
                                >
